...
/* Edited by: Michael Wehar */

var x = 1;
var updateFunction = function(){ x=0; };
$(document).on("updateEvent", updateFunction);
    ...
    $.fn.sudokuJS = function(opts) {
		...
		*/
		...
		var buildBoardCell = function(boardCell, id){
			...
			return "<div class='sudoku-board-cell'>" +
						...
						"<input type='text' pattern='\\d*' novalidate id='input-"+boardName+id+"' value='"+val+"'"+maxlength+">" +
						"<div id='input-"+boardName+id+"-candidates' class='candidates'>" + candidatesString + "</div>" +
					"</div>";
		};
		 ...
		 * -----------------------------------------------------------------*/
		 var updateUIBoardCell = function(cellIndex, opts){
			...
			//if(!(opts.mode && opts.mode === "only-candidates")){
				...
				$("#input-"+boardName+cellIndex)
					.val(newVal)
					.addClass("highlight-val");
			//}
			$("#input-"+boardName+cellIndex+"-candidates")
		...
		var uIBoardHighlightRemoveCandidate = function(cellIndex, digit){
			$("#input-"+boardName+cellIndex+"-candidates div:nth-of-type("+digit+")").addClass("candidate--to-remove");
		};
		...
		var uIBoardHighlightCandidate = function(cellIndex, digit){
			$("#input-"+boardName+cellIndex+"-candidates div:nth-of-type("+digit+")").addClass("candidate--highlight");
		};
		...
		var resetCandidates = function(updateUI){
			...
			for(var i=0; i <boardSize*boardSize;i++){
				if(board[i].val === null){
					...
					if(updateUI !== false)
						$("#input-"+boardName+i+"-candidates").html(buildCandidatesString(resetCandidatesList));
				} else if(updateUI !== false) {
						$("#input-"+boardName+i+"-candidates").html("");
				}
			}
		};
		...
		var keyboardNumberInput = function(input, id){
			...
			if (val > 0) { //invalidates Nan
				...
				for(var i=0; i < houses.length; i++){
					...
					if(indexInHouse(val, houses[i][temp[i]])){
						...
						$("#input-" + boardName+ alreadyExistingCellInHouseWithDigit + ", #input-"+boardName+id)
			...
			if($("#input-"+boardName+id).hasClass("board-cell--error"))
      ...
      if (contains(DIFFICULTIES, diff)) {
		...
		$boardInputs.on("keyup", function(e){
			var $this = $(this);
			var id = parseInt($this.attr("id").replace("input-"+boardName,""));
